const API_BASE = "https://healthraahi.onrender.com/api";
...
async function request(endpoint, { body, token, method = "POST" } = {}) {
  ...
  if (contentType.includes("application/json") && text) {
    try {
      ...
      const err = new Error("Server returned an invalid response. Please try again.");
...
export function lookupWorkerByHealthId(token, healthId) {
  return request(`/workers/lookup/${encodeURIComponent(healthId)}`, {
    token,
    method: "GET",
  });
}
...
export function getWorkerProfileForDoctor(token, healthId) {
  return request(`/workers/lookup/${encodeURIComponent(healthId)}/profile`, {
    token,
    method: "GET",
  });
}
...
export function verifyDoctor(token, doctorId, status, reason) {
  ...
  return request(`/doctors/${doctorId}/verify`, {
    token,
    method: "PATCH",
    body,
  });
...
export function registerForCamp(token, campId, timeSlotIndex, healthConcerns) {
  ...
  return request(`/camps/${encodeURIComponent(campId)}/register`, {
    token,
    body,
  });
}
...
export function cancelCampRegistration(token, regId) {
  return request(
    `/camps/my-registrations/${encodeURIComponent(regId)}/cancel`,
    { token, method: "PATCH" },
  );
}

export function getCampRegistrations(token, campId) {
  return request(`/camps/${encodeURIComponent(campId)}/registrations`, {
    token,
    method: "GET",
  });
}
...
export function updateCamp(token, campId, data) {
  return request(`/camps/admin/${encodeURIComponent(campId)}`, {
    token,
    body: data,
    method: "PATCH",
  });
...
export function cancelCamp(token, campId) {
  return request(`/camps/admin/${encodeURIComponent(campId)}/cancel`, {
    token,
    method: "PATCH",
  });
}
...
export function assignDoctorToCamp(token, campId, doctorId) {
  return request(`/camps/admin/${encodeURIComponent(campId)}/assign-doctor`, {
    token,
    body: { doctorId },
  });
}
...
export function getAdminCampRegistrations(token, campId) {
  return request(`/camps/admin/${encodeURIComponent(campId)}/registrations`, {
    token,
    method: "GET",
  });
}
...
export async function replaceDoctorVerificationDocument(
  token,
  documentId,
  file,
) {
  ...
  const res = await fetch(
    `${API_BASE}/doctor-verification-documents/${encodeURIComponent(documentId)}/replace`,
    {
      method: "PUT",
      headers: { Authorization: `Bearer ${token}` },
      body: formData,
...
export function deleteDoctorVerificationDocument(token, documentId) {
  return request(
    `/doctor-verification-documents/${encodeURIComponent(documentId)}`,
    { token, method: "DELETE" },
  );
}

export async function downloadDoctorVerificationDocumentFile(
  token,
  documentId,
) {
  const res = await fetch(
    `${API_BASE}/doctor-verification-documents/${encodeURIComponent(documentId)}/download`,
    {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    },
...
export function getAdminDoctorVerificationDocuments(token, doctorUserId) {
  return request(
    `/doctor-verification-documents/admin/${encodeURIComponent(doctorUserId)}`,
    { token, method: "GET" },
  );
}
...
export async function adminDownloadDoctorDocument(token, documentId) {
  const res = await fetch(
    `${API_BASE}/doctor-verification-documents/admin/doc/${encodeURIComponent(documentId)}/download`,
    {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    },
...
export function adminUpdateDocumentStatus(
  token,
  documentId,
  status,
  rejectionReason,
  ...
  return request(
    `/doctor-verification-documents/admin/doc/${encodeURIComponent(documentId)}/status`,
...
export function getWorkerDocumentById(token, documentId) {
  return request(`/worker-documents/${encodeURIComponent(documentId)}`, {
    token,
    method: "GET",
  });
}
...
export async function downloadWorkerDocumentFile(token, documentId) {
  const res = await fetch(
    `${API_BASE}/worker-documents/${encodeURIComponent(documentId)}/download`,
    {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    },
...
export function getClinicalRecordDocuments(token, recordId) {
  return request(`/clinical-record-documents/record/${recordId}`, {
    token,
    method: "GET",
  });
}
...
export async function downloadClinicalRecordDocument(token, documentId) {
  const res = await fetch(
    `${API_BASE}/clinical-record-documents/${documentId}/download`,
    {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    },
